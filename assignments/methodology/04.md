...
# 🛠 Methodology Assignment 3 - Solving Merge Conflicts

Before attempting this assignment, make sure to read [Lesson on GIT](https://dsc-capstone.org/2025-26/lessons/06). Submit this assignment by **Monday, November 3rd at 11:59PM**. Post questions with the assignment on Piazza.
...
{: .red }
**You'll _need_ to complete this assignment with a partner! Your partner doesn't need to be in your domain but its easiest if they are. If you can't find someone to work with, you can post on Piazza. **
...
Partner A should create a new **public** GitHub repository called `DSC180A-Methodology-3`. Click "Add a README file." Once the repository is created, Partner A should click the "Settings" button in the menu bar at the top, then the "Collaborators" button in the menu bar on the left, and then add Partner B as a collaborator. Now both Partner A and Partner B have full access to the repository.

---

### Step 2 – Populating the Repository with Files
Partner A should clone the repository locally. Then, they should:
1. Edit `README.md` so that, underneath the line with `# DSC180A-Methodology-3`, there's a single line of text with their full name.
1. Download [`lec02.ipynb`](https://dsc-capstone.org/2025-26/assignments/methodology/assets/lec02.ipynb) and place it in the repository. (Don't change anything about the notebook. Yes, this this the notebook used for Lecture 2 in DSC 10 – the point is to simulate a real-world problem!)
...
1. After using `git add` in one of the two ways mentioned above, Partner A should `git commit` and `git push` their changes. The repository at [github.com/<Partner A's username>/DSC180A-Methodology-3]() should now have two files – a `README.md` with `# DSC180A-Methodology-3` as the first line and Partner A's full name as the second line, and a `lec02.ipynb`. (It may also have a third file, `.gitignore` – this is fine.)
...
1. Open `lec02.ipynb` locally. Search for "PEMDAS". Edit the two cells underneath the "Python uses the typical order of operations – PEMDAS (BEDMAS? 🛏️)" heading to match what's shown below. Don't run the cells.
    <center><img src="https://dsc-capstone.org/2025-26/assignments/methodology/assets/m4-pB.png" width="700"></center>
1. `git add`, `git commit`, and `git push` their changes.
<!-- 1. `git add`, `git commit`, and `git push` their changes to the `partner-B` branch of the repository. Now, [https://github.com/<Partner A's username>/DSC180A-Methodology-3/tree/Partner-B]() should reflect the changes made by Partner B. -->
...
1. Open `lec02.ipynb` locally. Search for "PEMDAS". Edit the two cells underneath the "Python uses the typical order of operations – PEMDAS (BEDMAS? 🛏️)" heading to match what's shown below. Don't run the cells.
    <center><img src="https://dsc-capstone.org/2025-26/assignments/methodology/assets/m4-pA.png" width="700"></center>
1. `git add`, `git commit`, and try to `git push` their changes. After running `git push`, Partner A will see something like this in their Terminal:
    ```
    error: failed to push some refs to 'https://github.com/ubellur/DSC180A-Methodology-3.git'
...
1.  Partner A should open `README.md`. It'll look something like the following, just with different names and a different commit hash. (If it doesn't look like this, run `git config pull.rebase false` and then `git pull` again.)
    ...
    Partner A needs to edit `README.md` so that it doesn't contain `<<<<<<< HEAD`, `=======`, or `>>>>>>> 4cda707262d5798f4e1b423419220a924c79179a`. They should edit it so that `README.md` only contains two lines: one with `# DSC180A-Methodology-3` and another with the names of **both** partners separated with an "and" (e.g. `Umesh Bellur and Dilraj Gill`), no emails. If you're using VSCode, it may automatically detect that these lines were added by Git and let you choose which versions to keep.
...
1. Partner A now needs to fix the merge conflict in `lec02.ipynb`. **This will be more difficult**, because the inclusion of `<<<<<<< HEAD`, `=======`, `>>>>>>> 4cda707262d5798f4e1b423419220a924c79179a` prevent `lec02.ipynb` from being a valid notebook. If you try to open it, you'll see the following in your Terminal:
    ```
    [W 14:30:34.302 NotebookApp] 400 GET /api/contents/lec02.ipynb?type=notebook&_=1699558234145 (::1): Unreadable Notebook: /Users/ubellur/Desktop/DSC180A-Methodology-3/lec02.ipynb NotJSONError('Notebook does not appear to be JSON: \'{\\n "cells": [\\n  {\\n   "cell_type": "c...')
    ```
    ...
    <center><img src="https://dsc-capstone.org/2025-26/assignments/methodology/assets/m4-lec02.png" width="700"></center>
    ...
    <center><img src="https://dsc-capstone.org/2025-26/assignments/methodology/assets/m4-lec02-merged.png" width="700"></center>
...
1. Finally, Partner B should submit the Git repository `<Partner A's username>/DSC180A-Methodology-3` to Gradescope. (As long as Partner A correctly added Partner B as a collaborator, Partner B will be able to submit, even though the repository is "owned" by Partner A). 
